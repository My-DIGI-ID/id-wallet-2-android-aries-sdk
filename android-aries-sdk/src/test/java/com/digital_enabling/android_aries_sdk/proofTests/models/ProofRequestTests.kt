...
class ProofRequestTests {
    @Test
    @DisplayName("Serialized ProofRequest has all keys if the values are not null.")
    fun serialization_works() {
        //Arrange
        val testObject = ProofRequest()
        testObject.name = "testName"
        testObject.version = "testVersion"
        testObject.nonce = "testNonce"
        ...
        val actual = Json.decodeFromString<ProofRequest>(serializedObject)

        //Assert
        Assertions.assertEquals(testObject.name, actual.name)
        Assertions.assertEquals(testObject.version, actual.version)
        Assertions.assertEquals(testObject.nonce, actual.nonce)
    ...
    @Test
    @DisplayName("Serialized ProofRequest works even if some properties do not exist.")
    ...
    @Test
    @DisplayName("Serialized ProofRequest works even if no properties exist.")
